...
const AdminLogin = ({ setIsAdminLoggedIn }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const backendUrl =
        import.meta.env.VITE_BACKEND_URL || "http://localhost:8090"; // Fallback URL for local development

      const response = await fetch(
        `${backendUrl}/portfolio-backend1/AdminLoginServlet`, // Use the backend URL from .env
        {
          method: "POST",
          headers: {
            "Content-Type": "application/x-www-form-urlencoded",
