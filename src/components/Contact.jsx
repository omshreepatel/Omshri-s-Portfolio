...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await fetchApi(
        "/portfolio-backend1/ContactServlet",
        "POST",
        formData
      );
      ...
      if (result.status === "success") {
        ...
        showErrorAlert(result.message || "Message sending failed");
      }
    } catch (error) {
      showErrorAlert("Failed to connect to server. Please try again later.");
    } finally {
  ...
  return (
    <Container id="contact" className="contact-section my-5">
      ...
      {/* Social Media Section remains same */}
      <div className="social-media mt-5 text-center">
        <h4 className="mb-3">Follow Me</h4>
        <div className="d-flex justify-content-center gap-4">
          <a
